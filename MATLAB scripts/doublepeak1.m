...
eventenergy = 30;
%%
ftau = sprintf('../old/tau_%dTeV.csv',eventenergy);
fhad = sprintf('../old/had_%dTeV.csv',eventenergy);
...
function [dpeak] = isdoublepeak(had,tau,lay,col,cell)
    ...
    if (angle > 35 && angle <45) || length(hit_sum)<100 
        dpeak = -1;
        return
    end
    ...
    if Results(1,3)/Results(2,3) > 3.5 || Results(2,3)/Results(1,3) > 3.5 ||...
       ra > 8 || ra < 0.125 || Results(1,4)<3 || Results(2,4)<3 || ...
